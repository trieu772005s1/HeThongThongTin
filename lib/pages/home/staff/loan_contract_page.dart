...
class _LoanContractPageState extends State<LoanContractPage> {
  ...
  Future<void> _submit() async {
    ...
    if (amount <= 0 || termMonths <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Số tiền và kỳ hạn phải lớn hơn 0')),
      );
      return;
    }
    ...
    try {
      ...
      if (_createSchedule) {
        final bool iAmOwner = await FirebaseFirestore.instance
            .runTransaction<bool>((tx) async {
              final snap = await tx.get(newLoanRef);
              final data = snap.data();
              final bool already =
                  (data != null && data['scheduleCreated'] == true);
              if (already) return false;
              tx.update(newLoanRef, {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Tạo hợp đồng thành công')));
      Navigator.pushReplacementNamed(
        context,
        '/loanDetail',
        arguments: {'loanId': newLoanRef.id},
      );
    } catch (e, st) {
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        '${_monthFormat.format(monthlyRounded)} VND',
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                          'Tổng trả sau $term tháng',
                          style: TextStyle(color: Colors.grey.shade700),
                        ),
                      ),
                      Text(
                        '${_currencyFormat.format(totalRepay)} VND',
                        style: const TextStyle(fontWeight: FontWeight.bold),
                      ),
                    ],
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Tạo hợp đồng'), elevation: 1),
                              ...
                              children: [
                                Checkbox(
                                  ...
                                  onChanged: (v) => setState(
                                    () => _createSchedule = v ?? true,
                                  ),
                                ),
                                const SizedBox(width: 6),
                                Expanded(
                                  child: Text(
                                    'Tạo lịch thanh toán tự động (hàng tháng)',
                                    style: TextStyle(
                                ...
                                Tooltip(
                                  message:
                                      'Bỏ chọn nếu không muốn tạo schedule tự động.',
                                  child: Icon(
                                    Icons.info_outline,
                                    size: 18,
