...
class LoanDetailPage extends StatelessWidget {
          ...
          return FutureBuilder<DocumentSnapshot>(
            ...
            builder: (context, userSnap) {
              ...
              if (userSnap.connectionState == ConnectionState.done && userSnap.hasData && userSnap.data!.exists) {
                ...
                customerName = (u['full_name'] ?? u['fullName'] ?? u['name'] ?? customerName ?? '').toString();
                phone = (u['phone'] ?? u['phoneNumber'] ?? phone ?? '').toString();
              }
              ...
              return ListView(
                ...
                children: [
                  ...
                  _infoCard([
                    _row('Họ tên', customerName.isNotEmpty ? customerName : (customerId.isNotEmpty ? customerId : '-')),
                  ...
                  _infoCard([
                    ...
                    _row('Kỳ hạn', '${data['termMonths'] ?? data['tenure'] ?? '-'} tháng'),
                    _row('Lãi suất', '${data['interestRate'] ?? data['interest'] ?? '-'} %'),
                    _row('Ngày bắt đầu', _tsDate(data['startDate'] ?? data['createdAt'])),
  ...
  static String _money(dynamic v) {
    num val = 0;
    if (v is num) val = v;
    else if (v is String) val = num.tryParse(v.replaceAll(',', '')) ?? 0;
