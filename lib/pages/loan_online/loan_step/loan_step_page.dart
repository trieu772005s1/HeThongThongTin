...
import 'steps/loan_step2_1.dart';

class LoanStepPage extends StatefulWidget {
  final String name;
  final String idNumber;

  const LoanStepPage({super.key, required this.name, required this.idNumber});
...
class _LoanStepPageState extends State<LoanStepPage> {
  int _currentStep = 2; // Bắt đầu từ bước 2 vì bước 1 đã xác thực ở ngoài

  late String _userName;
  late String _userIdNumber;

  @override
  void initState() {
    super.initState();
    _userName = widget.name;
    _userIdNumber = widget.idNumber;
  }
  ...
  void _handleStepStart(int step) async {
    ...
    if (step == 2) {
      final completed = await Navigator.push<bool>(
        context,
        MaterialPageRoute(
          builder: (_) => LoanStep2_1(
            initialName: _userName,
            initialIdNumber: _userIdNumber,
          ),
        ),
      );
  ...
  Widget _buildStepWithLine({required int step, required String title}) {
    final bool done = step < _currentStep || (step == 1); // step 1 luôn done
    final bool isCurrent = step == _currentStep;
    const colorDone = Color(0xFF1976D2);
